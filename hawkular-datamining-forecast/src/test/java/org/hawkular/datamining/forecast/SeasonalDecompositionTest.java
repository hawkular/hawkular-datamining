...
import org.hawkular.datamining.forecast.model.AbstractTest;
import org.hawkular.datamining.forecast.model.SimpleMovingAverageTest;
 ...
 * @author Pavol Loffay
 */
public class SeasonalDecompositionTest extends AbstractTest {
    ...
    public SeasonalDecompositionTest() {
        long counter = 0;
    ...
    public void testOnThreeSeasons() throws IOException {
        SeasonalDecomposition decomposition = new SeasonalDecomposition(testData, periods);
        decomposition.decompose();

        int nullsAtStart = SimpleMovingAverageTest.nullAt(decomposition.getTrend(), true);
        int nullsAtEnd = SimpleMovingAverageTest.nullAt(decomposition.getTrend(), false);
        Assert.assertTrue(nullsAtStart == 2);
        Assert.assertTrue(nullsAtEnd == 2);

        nullsAtStart = SimpleMovingAverageTest.nullAt(decomposition.getRandom(), true);
        nullsAtEnd = SimpleMovingAverageTest.nullAt(decomposition.getRandom(), false);
        Assert.assertTrue(nullsAtStart == 2);
        Assert.assertTrue(nullsAtEnd == 2);

        nullsAtStart = SimpleMovingAverageTest.nullAt(decomposition.getSeasonal(), true);
        nullsAtEnd = SimpleMovingAverageTest.nullAt(decomposition.getSeasonal(), true);
        ...
        for (int i = 0; i < decomposition.getSeasonal().size(); i++) {
            Assert.assertEquals(seasonal[i % periods], decomposition.getSeasonal().get(i).getValue(), 0.000001);
        }
        ...
        for (int i = 0; i < decomposition.getSeasonal().size(); i++) {
            if (random[i] == null) {
                if (decomposition.getRandom().get(i).getValue() != null) {
                    Assert.fail();
                }
                continue;
            }
            Assert.assertEquals(random[i], decomposition.getRandom().get(i).getValue(), 0.000001);
        }
    }
    ...
    public void testOnShorterSeries() {
        ...
        for (int i = 0; i < decomposition.getSeasonal().size(); i++) {
            ...
            Assert.assertEquals(trend[i], decomposition.getTrend().get(i).getValue(), 0.000001);
        }
