...
public class TripleExponentialSmoothingTest extends AbstractTest {
    ...
    public void testEmpty() {
        try {
            ...
            TripleExponentialSmoothing.optimizer(12).minimizedMSE(empty);
            Assert.fail();
        } catch (Throwable ex) {
            // ok
        }
    ...
    public void testMinimalPoints() throws IOException {
        ...
        try {
            optimizer.minimizedMSE(rModel.getData().subList(0, minimalPoints - 1));
            Assert.fail();
        } catch (IllegalArgumentException ex) {
            // ok
        }
    ...
    public void testSineLowVarMedium() throws IOException {
        ...
        assertThat(seasonalStat.getMse())
                .as(String.format("rModel: %s\nMy: %s\n%s", rModel,seasonalModel, seasonalStat))
                .isGreaterThanOrEqualTo(rModel.getMse()*ACCURACY_LOW)
                .isLessThanOrEqualTo(rModel.getMse()*ACCURACY_HIGH);
    }
    ...
    public void testSineLowVarTrend() throws IOException {
        ...
        assertThat(initStatistics.getMse())
                .as(String.format("rModel: %s\nMy: %s\n%s", rModel,seasonalModel, initStatistics))
                .isGreaterThanOrEqualTo(rModel.getMse()*ACCURACY_LOW)
                .isLessThanOrEqualTo(rModel.getMse()*ACCURACY_HIGH);
    }
    ...
    public void testAustourists() throws IOException {
        ...
        assertThat(seasonalStat.getMse())
                .as(String.format("rModel: %s\nMy: %s\niniStat: %s", rModel,seasonalModel, seasonalStat))
                .isGreaterThanOrEqualTo(rModel.getMse()*ACCURACY_LOW)
                .isLessThanOrEqualTo(rModel.getMse()*ACCURACY_HIGH);
    }
    ...
    public void testPrediction() throws IOException {
        ...
        for (int i = 0; i < nAhead; i++) {
            ...
            assertThat(forecast.get(i).getValue())
                .isGreaterThanOrEqualTo(expectedFromR[i]*ACCURACY_LOW)
                .isLessThanOrEqualTo(expectedFromR[i]*ACCURACY_HIGH);
        }
    }
