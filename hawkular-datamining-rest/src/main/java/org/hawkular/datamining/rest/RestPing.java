...
@Api(value = "/", description = "Info", tags = "Info")
public class RestPing {
    ...
    private JaxRsInfo jaxRsInfo;

    @PostConstruct
    private void init() {
        jaxRsInfo = getEndpoints((ResourceMethodRegistry)dispatcher.getRegistry());
        Collections.sort(jaxRsInfo.getEndpoints());
    }

    @GET
    @Path(RestPing.URL)
    @ApiOperation(value = "Returns complete list of REST endpoints.")
    public Response ping(@Context Dispatcher dispatcher) {
        return Response.status(Response.Status.OK).entity(jaxRsInfo).build();
    }

    private JaxRsInfo getEndpoints(ResourceMethodRegistry resourceMethodRegistry) {
        JaxRsInfo jaxRsInfo = new JaxRsInfo();
        ...
        for (Map.Entry<String, List<ResourceInvoker>> entry : resourceMethodRegistry.getBounded().entrySet()) {
            ...
            if (jaxRsResource.getMethods().size() > 0) {
                jaxRsInfo.addJaxRsResources(jaxRsResource);
            }
        }
